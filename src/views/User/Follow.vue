<template>
  <el-card :header="`${who}的关注`">
    <div v-show="getting" v-loading="getting" element-loading-text="加载中" style="min-height: 100px;"></div>

    <div v-show="!getting">
      <el-empty v-if="r.users.length === 0" :description="`${who}还没有关注用户哦～`" />

      <div v-else class="follow-page">
...
function getFollows() {
  getting.value = true
  API.GetFollows(queryId.value, curPage.value, timestamp.value)
    .then((res) => {
      if (res.code !== 0) {
