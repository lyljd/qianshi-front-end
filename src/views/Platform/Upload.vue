<template>
  <el-tabs class="pf-container">
    <el-tab-pane label="视频投稿" v-if="hasUploaded === '' || hasUploaded === 'video'">
      ...
      <div class="v-container" v-else>
        <div style="margin-bottom: 10px;">视频上传进度</div>
        ...
        <div class="setting">
          ...
          <div>
            ...
            <el-upload :on-change="onCoverUploadChange" :before-upload="beforeCoverUpload"
              :on-remove="onCoverUploadRemove" :on-progress="onCoverUploadProgress" :on-success="onCoverUploadSuccess"
              :on-error="onCoverUploadError" ref="coverUpload" action="/api/resource/cover" accept="image/*"
    ...
    <el-tab-pane label="专栏投稿" v-if="hasUploaded === '' || hasUploaded === 'read'">
...
function onVideoUploadProgress(event: any) {
  ...
  console.log("视频当前上传进度：" + videoUploadPercent.value + " / 100")
}
...
<style>
.pf-container .pg .el-progress__text {
...
.pf-container .pgf .el-progress__text {
  font-size: 14px !important;
  display: flex;
  justify-content: right;
  margin-left: -30px;
...
.pf-container .el-tabs__nav {
  z-index: inherit;
}

.pf-container .el-tabs__nav-wrap::after {
  z-index: inherit;
}

.pf-container .el-form-item {
  margin-bottom: 10px;
}
</style>