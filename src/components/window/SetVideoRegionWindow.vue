<template>
  <el-dialog v-model="mainWindowVisible" width="33%" title="设置视频分区" class="srw" :close-on-click-modal="false"
    ...
    <div class="body">
      <el-card style="max-height: 33vh; overflow: auto;">
        <div v-if="data.length > 0" v-for="(r, idx) in data" class="row"
          ...
          <el-input :readonly="saveLoading || idx < dataCopy.length" @focus="activeIdx = idx" :id="`r-slug-${r.slug}`"
            v-model="r.slug" placeholder="slug" :class="saveLoading || idx < dataCopy.length ? 'slug-ban-input' : ''" />
        </div>
...
async function save() {
  ...
  if (store.switchAsk) {
    ...
    await ManageAPI.setVideoRegions(data)
      .then((res => {
        if (res.code !== 0) {
